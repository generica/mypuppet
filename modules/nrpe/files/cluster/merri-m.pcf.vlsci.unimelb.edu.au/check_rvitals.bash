...
### /etc/puppet/modules/nrpe/files/cluster/merri-m.pcf.vlsci.unimelb.edu.au/check_rvitals.bash
############################################################################
...
bad=$(sudo /opt/xcat/bin/rvitals ipmis leds 2>&1| grep -v "No active error LEDs detected" | awk -F: '{print $1}' | sort | uniq)
count=$(echo $bad | wc -w)
