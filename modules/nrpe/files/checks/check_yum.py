...
class YumTester:
    ...
    def get_all_updates(self):
        ...
        for line in output:
            # Yum package truncation workaround mentioned above applied here
            if len(line) > 39:
                #if re_package_format_truncated.match(line):
                if not ( "Loaded plugins" in line or \
                         re.search("Loading .+ from ", line) or \
                         re.search(" \* ", line) or \
                         re.search("Skipping", line) or \
                         "Limiting package lists to security relevant ones" in line ):
                    count += 1
            else:
